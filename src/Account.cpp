...
std::string Account::getJwtToken( const std::string& url ) const
{
    ...
    try
    {
        ...
        std::string accountHeader = "NEXDEX-STARKNET-ACCOUNT: %1";
        {
            std::ostringstream s;
            s << address;
        ...
        std::string signatureHeader = "NEXDEX-STARKNET-SIGNATURE: [\"%1\", \"%2\"]";
        replace( &signatureHeader, "%1", signature.first );
        replace( &signatureHeader, "%2", signature.second );

        std::string hashHeader = "NEXDEX-STARKNET-MESSAGE-HASH: %1";
        replace( &hashHeader, "%1", hash );

        std::string nowHeader = "NEXDEX-TIMESTAMP: %1";
        std::string strNow = std::to_string( now );
        replace( &nowHeader, "%1", strNow );

        std::string expiryHeader = "NEXDEX-SIGNATURE-EXPIRATION: %1";
