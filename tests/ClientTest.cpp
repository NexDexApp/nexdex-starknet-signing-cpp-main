...
int postOrder(const signer::Account& account, const std::string& jwtToken)
{
    ...
    try
    {
        ...
        // Set the request URL
        std::string nexdex_http_url = "https://api.testnet.nexdex.trade/v1";
        std::string path = "/orders";
        request.setOpt( new curlpp::options::Url( nexdex_http_url + path ) );
...
int main()
{
    ...
    const std::string jwtToken = account.getJwtToken( "https://api.testnet.nexdex.trade/v1/auth" );
